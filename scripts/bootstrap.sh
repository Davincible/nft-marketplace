...
echo "Bootstrap workspace dir: ${workspace}"
echo "Running as $USER"
ls -lah /home/docker
ls -lah /home/docker/storage
...
function init_validator() {
	...
	validatorAddr=$(grep 'Public address of the key' <"${workspace}/storage/${node_id}Info" | awk '{print $6}')
...
function generate_genesis() {
	...
	echo "Using init holders: '${INIT_HOLDER_ADDRESSES}'"

	sed "s/{{INIT_HOLDER_ADDRESSES}}/${INIT_HOLDER_ADDRESSES}/g" "${workspace}/genesis/init_holders.template" |
		sed "s/{{INIT_HOLDER_BALANCE}}/${INIT_HOLDER_BALANCE}/g" > "${workspace}/genesis/init_holders.js"

	cat "${workspace}/genesis/init_holders.js"
